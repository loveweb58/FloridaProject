...
class EditCombinedCaseModal extends React.Component {
  state = {
    error: null,
    combined_case_name: '',
  ...
  handleSubmit = () => {
    if(this.props.selectedCombinedCase.personId && this.state.combined_case_name && this.state.combined_case_description){
      if(this.state.show_case_modal) {
        this.setState({show_case_modal: false});
      }else{
        this.setState({ error: null });
        this.props.editCombinedCase(this.props.selectedCombinedCase.id, {
          personId: this.props.selectedCombinedCase.personId,
  ...
  render() {
    ...
    return (
      <Modal isOpen={modalOpen} toggle={toggleModal} size="lg" backdrop="static">
        <ModalHeader toggle={toggleModal}>Edit CombinedCase</ModalHeader>
        {show_case_modal ?
            <ModalBody>
              <Row>
                <Col xs={6}>
